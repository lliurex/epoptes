...
class EpoptesGui(object):
    ...
    def quit_ip_service(self):
        try:
            var_epoptes_ip_sever="EPOPTES_IP_SERVER"
            import xmlrpc.client as n4dclient
            import ssl
            context=ssl._create_unverified_context()
            c=n4dclient.ServerProxy("https://localhost:9779",context=context,allow_none=True)
            ret=c.get_variable("EPOPTES_IP_SERVER")
            if ret["return"]==None:
                pass
            else:
                solved=c.release_ip((self.n4d_user,self.n4d_password),"EpoptesVariables",ret["return"],var_epoptes_ip_sever)
                print ("Deleting this IP: %s"%ret["return"])
    ...
    def reverse_connection(self, cmd, *args):
        ...
        if class_ip == None:
            class_ip = "server"
        #TEST IF IS A DESKTOP LLIUREX CONTROL
        import xmlrpc.client as n4dclient
        import ssl
        context=ssl._create_unverified_context()
        c=n4dclient.ServerProxy("https://localhost:9779",context=context,allow_none=True)
