...
class EpoptesGui(object):
    ...
    def __init__(self):
        ...
        saved_clients, groups = config.read_groups(os.path.expanduser('~/.config/epoptes/groups.json'),True)
        for grp in groups:
    ...
    def blockInternet(self,widget):
		...
		if client[C_INSTANCE].type in fat_clients:
			...
			ip=client[C_INSTANCE].users.keys()[0].split(":")[0]
			print
		...
		if client[C_INSTANCE].type=="thin":
			...
			key=client[C_INSTANCE].users.keys()[0]
